...
**Product name:** 扇子 (Shanzi), formerly Paperweave. The repository, MCP registration key `paperweave`, protocol identifiers, native bundle ID and existing vault paths remain stable for compatibility. Present the product as 扇子 to users. On Windows, 0.2.2 repairs the legacy uninstall-before-install upgrade; use the new installer directly and preserve the registered installation directory so existing bundled MCP paths remain valid. Do not ask users to delete research data or manually uninstall the old build first. Interactive maintenance asks before closing this installation's desktop/bundled services; silent/passive maintenance exits with code 10 if those processes are active. Coordinate existing embedded sessions before stopping them; external CLI processes must remain untouched.
