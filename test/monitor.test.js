...
test("monitor distinguishes missing, partial and unknown evidence; waiting remains visible when stale", async () => {
  ...
  try {
    ...
    assert.equal(
      (await scanMonitor(config)).providers[0].availability,
      "missing",
    );
    ...
    for (const [i, row] of rows.entries())
      await fs.writeFile(
        path.join(home, "sessions", `${i + 1}.json`),
        JSON.stringify(row),
      );
    await fs.writeFile(path.join(home, "sessions/9.json"), '{"status":');
    const before = await fs.readFile(
      path.join(home, "sessions/1.json"),
      "utf8",
    );
    ...
    assert.equal(
      data.sessions.find((s) => s.id === "claude:1").state,
      "needs_approval",
    );
    assert.equal(data.sessions.find((s) => s.id === "claude:1").stale, true);
    assert.equal(
      data.sessions.find((s) => s.id === "claude:3").state,
      "unknown",
    );
    assert.equal(data.sessions.find((s) => s.id === "claude:4").state, "stale");
    assert.equal(
      (await scanMonitor(config, { now, currentProject: true })).sessions
        .length,
      4,
    );
    assert.equal(
      await fs.readFile(path.join(home, "sessions/1.json"), "utf8"),
      before,
    );
  } finally {
...
test("monitor uses transcript evidence for activity without inferring approvals or exposing content", async () => {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "paperweave-monitor-"));
  try {
    const cwd = "C:\\research\\fixture",
      slug = cwd.replace(/[:\\/_.]/g, "-");
    await fs.mkdir(path.join(home, "sessions"));
    ...
    for (const [i, stop] of ["end_turn", "tool_use"].entries()) {
      ...
      await fs.writeFile(
        path.join(home, "projects", slug, `fixture${i}.jsonl`),
        JSON.stringify({
          type: "assistant",
          message: {
            stop_reason: stop,
            content: [
              {
                type: "tool_use",
                name: "Read",
                input: "DO NOT EXPOSE TRANSCRIPT",
              },
            ],
          },
        }) + '\n{"partial":',
    ...
    assert.equal(
      data.sessions.find((s) => s.id === "claude:2").state,
      "needs_attention",
    );
    assert.equal(
      data.sessions.some((s) => s.state === "needs_approval"),
      false,
    );
    assert.equal(
      JSON.stringify(data).includes("DO NOT EXPOSE TRANSCRIPT"),
      false,
    );
  } finally {
    await fs.rm(home, { recursive: true, force: true });
