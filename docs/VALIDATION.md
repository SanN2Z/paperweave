...
GitHub Actions is configured for Windows, Linux and macOS. Its results should be checked on the repository's Actions tab; a local Windows pass alone is not evidence that all three platforms passed remotely.
...
- Native macOS / Linux manual usage beyond any reported Actions results.
