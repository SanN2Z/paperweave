...
Latest desktop addition: **23 automated tests passed**. The installed Tauri/WebView2 application passed `scripts/desktop-check.js` with Node removed from its PATH: bundled PTY input/output, real Chinese clipboard paste, four-page PDF rendering, independent native monitor, pin/collapse, hide/restore retaining the same document and terminal, and a real bundled stdio MCP connection from an unrelated directory. Native testing exposed and fixed extended Windows path handling and application-command ACLs. These checks use synthetic research and session fixtures. They do not establish clean-VM installation, code signing, external terminal focus or macOS/Linux desktop support.
